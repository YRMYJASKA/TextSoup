...
unsigned lineArea =
    0;  // Variable used to draw more lines that the screens height allows
...
int main(int count, char* option[]) {
	...
	init_pair(1, COLOR_BLACK,
		  COLOR_WHITE);  // Inverted colour pair (cursor)
	...
	while (running) {
		...
		switch (key) {
			...
			case KEY_BACKSPACE:
				if (CURS_X > 0) {
					LineBuffer[CURS_Y].erase(
					    CURS_X - 1, 1);  // Delete the
							     // character under
							     // the cursor
					CURS_X--;  // Move the cursor to the
						   // left
				} else {
					if (CURS_Y > 0) {  // Checking we are
							   // not deleting the
							   // first line
						LineBuffer[CURS_Y - 1]
						    .pop_back();  // Delete the
			...
			case int('\n'):
				...
				LineBuffer.insert(
				    ...
				    LineBuffer[CURS_Y].substr(
					CURS_X, LineBuffer[CURS_Y].length()));
			...
			case KEY_UP:
				if (CURS_Y != 0) {
					CURS_Y--;
					if (CURS_X + 1 >=
					    LineBuffer[CURS_Y].length())
						CURS_X = LineBuffer[CURS_Y]
							     .length() -
							 1;
			...
			case 9:
				for (int i = 0; i < 4; i++) {
					LineBuffer[CURS_Y].insert(
					    LineBuffer[CURS_Y].begin() + CURS_X,
					    char(' '));
					CURS_X += 1;
				}
				break;
		...
		if (i == CURS_Y) {  // If this line has te cursor on it draa it
			...
			for (unsigned int x = 0; x < LineBuffer[i].length();
				...
				if (x == CURS_X) {
					...
					mvprintw(
					    z + TOP_PADDING, x + LEFT_PADDING,
					    "%c",
					    LineBuffer[i].at(x));  // Draw the
								   // regular
								   // character
				}
			}
		} else {  // If not the cursor line draw without special
			  // handling
			mvprintw(z + TOP_PADDING, LEFT_PADDING,
				 LineBuffer[i].c_str());
		}
	...
	while (getline(iFILE, line)) {
		if (firstline) {
			...
			LineBuffer.push_back(
			    line + " ");  // Append every line to the LineBuffer
		}
	}
...
void getLocation() {
	ifstream iFILE("/etc/textSoup/location");  // File that contains the
